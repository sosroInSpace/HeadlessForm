        ...
        <form action="" name={form_name + "-form"} onSubmit={this.handleSubmit.bind(this)}>
            <div className="fieldset-double">
             ...
             <fieldset>
                ...
                {para_fields ? 
                    // render data if available
                    para_fields.map((question, index) => {
                        return <fieldset key={index}><label>{question}</label><textarea type="textarea" name={question.replace(/ /g, "-")}></textarea></fieldset>;
                    })
                ...
                {select_name ? <fieldset><label>{select_name}</label>{select_option ? select_option.map((question, index) => { return <div class="checkbox"><input type="checkbox" id={question} /><label for={question}>{question}</label></div> }) : "" }</fieldset> : ""}
